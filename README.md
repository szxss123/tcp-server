...
- 使用 epoll 管理监听 Socket 与客户端 Socket 事件
- 累计读取分片到达的 HTTP 请求头，并限制请求头最大为 8KB
- 使用独立 HTTP 解析器校验请求行和请求头
- 对根路径 GET 请求返回 `200 OK`，未知路径返回 `404 Not Found`
- 对畸形请求返回 `400 Bad Request`，非 GET 请求返回 `405 Method Not Allowed`
...
主线程负责监听端口并接受连接，将客户端处理任务提交到共享队列。固定数量的工作线程通过条件变量等待任务，从队列取出连接后完成分片请求头累计读取、HTTP 解析、路由分流、响应构造和发送。客户端描述符由 RAII 对象负责关闭，线程池析构时会唤醒并回收全部工作线程。
...
- 将 HTTP 协议解析从网络 I/O 逻辑中解耦，支持分片请求头累积、请求行校验及 200、400、404、405 状态响应，并设置请求头大小上限以降低异常请求风险。
